...
class writingScreenData {
  ...
  factory writingScreenData.fromJson(Map<String, dynamic> json) {
    return writingScreenData(
...
Future<writingScreenData> fetchwritingScreenData() async {
  final response = await http.get(Uri.parse('https://api.example.com'));

  if (response.statusCode == 200) {
    final jsonData = jsonDecode(response.body);
    return writingScreenData.fromJson(jsonData);
  } else {
    throw Exception('Failed to fetch user data');
  }
}

class WritingHeaderData {
  final String title;
  ...
  factory WritingHeaderData.fromJson(Map<String, dynamic> json) {
    return WritingHeaderData(
      title: json['title'],
...
Future<WritingHeaderData> fetchwritingHeaderData() async {
  final response = await http.get(Uri.parse('https://api.example.com'));

  if (response.statusCode == 200) {
    final jsonData = jsonDecode(response.body);
    return WritingHeaderData.fromJson(jsonData);
  } else {
    throw Exception('Failed to fetch user data');
  }
}