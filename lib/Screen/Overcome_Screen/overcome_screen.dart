...
class OvercomeData {
  ...
  factory OvercomeData.fromJson(Map<String, dynamic> json) {
    return OvercomeData(
...
Future<OvercomeData> fetchOvercomeData() async {
  final response = await http.get(Uri.parse('https://api.example.com'));

  if (response.statusCode == 200) {
    final jsonData = jsonDecode(response.body);
    return OvercomeData.fromJson(jsonData);
  } else {
    throw Exception('Failed to fetch user data');
  }
}

class OvercomeHeaderData {
  final String title;
  ...
  factory OvercomeHeaderData.fromJson(Map<String, dynamic> json) {
    return OvercomeHeaderData(
      title: json['title'],
...
Future<OvercomeHeaderData> fetchOvercomeHeaderData() async {
  final response = await http.get(Uri.parse('https://api.example.com'));

  if (response.statusCode == 200) {
    final jsonData = jsonDecode(response.body);
    return OvercomeHeaderData.fromJson(jsonData);
  } else {
    throw Exception('Failed to fetch user data');
  }
}
